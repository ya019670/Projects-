...
class _VotePageState extends State<VotePage> {
                ...
                child: Padding(
                  ...
                  child: Container(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        _showConfirmationDialog(candidate); // Pass the candidate information
                      },
  ...
  void _showConfirmationDialog(Map<String, dynamic> candidate) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return Directionality(
          ...
          child: AlertDialog(
            ...
            content: const Text('هل أنت متأكد أنك تريد التصويت لهذا المرشح؟'),
            actions: [
              TextButton(
                onPressed: () async {
                  // Increment votes in the database
                  await CandidateDatabaseHelper.instance.updateVotes(
                    candidate['id'],
                    (candidate['votes'] ?? 0) + 1,
                  );

                  Navigator.of(context).pop(); // Close the dialog
                  _showVoteConfirmationSnackbar();
                },
  ...
  void _showVoteConfirmationSnackbar() {
    const snackBar = SnackBar(
      content: Text('تم التصويت بنجاح!'),
      duration: Duration(seconds: 2),
      backgroundColor: Colors.redAccent, // Change background color
    );
